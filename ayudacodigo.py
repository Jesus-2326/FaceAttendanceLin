    ...
    for archivo in archivos:
        ...
        for user_id_str, info in registros.items():
            try:
                ...
                hora_esperada = '08:00:00'
                estado = 'Asis'

                cursor.execute("""
                    INSERT INTO asistencias_controladas 
                        (usuario_id, fecha, hora_esperada, hora_entrada, hora_salida, estado)
                    VALUES 
                        (%s, %s, %s, %s, %s, %s)
                    ON DUPLICATE KEY UPDATE 
                        hora_entrada = VALUES(hora_entrada),
                        hora_salida = VALUES(hora_salida),
                        hora_esperada = VALUES(hora_esperada),
                        estado = VALUES(estado)
                """, (user_id, fecha, hora_esperada, entrada, salida, estado))

                registros_subidos += 1

            except Exception as e:
                errores.append(f"Error al insertar registro de usuario {user_id_str} en {archivo}: {e}")
                errores_archivo += 1
